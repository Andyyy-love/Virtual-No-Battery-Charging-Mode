...
int charger_dev_set_pd_adapter(struct charger_device *charger_dev, int *pd_adapter)
{
        if (charger_dev != NULL && charger_dev->ops != NULL &&
                                           charger_dev->ops->set_pd_adapter)
                return charger_dev->ops->set_pd_adapter(charger_dev, pd_adapter);

        return -EOPNOTSUPP;
}
EXPORT_SYMBOL(charger_dev_set_pd_adapter);
...
int charger_dev_set_vreg_ft(struct charger_device *chg_dev, bool temp)
{
	if (chg_dev != NULL && chg_dev->ops != NULL &&
	    chg_dev->ops->set_vreg_ft)
		chg_dev->ops->set_vreg_ft(chg_dev, temp);

	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(charger_dev_set_vreg_ft);
