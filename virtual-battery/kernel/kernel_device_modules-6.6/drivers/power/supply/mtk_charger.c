...
extern void lenovo_charge_create_sysfs(struct power_supply *psy);
extern int get_battery_protection_setting(void);
extern int get_input_suspend(void);
extern int get_battery_protection_setting_eu(void);
extern int get_charging_enable(void);
extern void sysfs_register_notifier(struct notifier_block *nb);
				...
				struct device *dev)
{
	...
	if (of_property_read_u32(np, "recharge_offset", &val) >= 0)
		info->data.recharge_offset = val;
	else {
		chr_err("use default BATTERY_CV:%d\n", RECHARGE_OFFSET);
		info->data.recharge_offset = RECHARGE_OFFSET;
	}

	if (of_property_read_u32(np, "battery_recharge", &val) >= 0)
		info->data.recharge_vol = val;
	else {
		chr_err("use default BATTERY_CV:%d\n", RECHARGE_VOL);
		info->data.recharge_vol = RECHARGE_VOL;
	}
	...
	if (of_property_read_u32(np, "pd_max_charger_voltage", &val) >= 0)
		info->data.pd_max_charger_voltage= val;
	else {
		chr_err("use default V_CHARGER_MAX:%d\n", V_CHARGER_MAX);
		info->data.pd_max_charger_voltage = V_CHARGER_MAX;
	}
	info->data.max_charger_voltage_setting = info->data.max_charger_voltage;
	...
	if (of_property_read_u32(np, "jeita_temp_above_t4_term", &val) >= 0)
		info->data.jeita_temp_above_t4_term = val;
	else {
		chr_err("use default term:%d\n", ITERM);
		info->data.jeita_temp_above_t4_term = ITERM;
	}

	if (of_property_read_u32(np, "jeita_temp_t3_to_t4_term", &val) >= 0)
		info->data.jeita_temp_t3_to_t4_term = val;
	else {
		chr_err("use default term:%d\n", ITERM);
		info->data.jeita_temp_t3_to_t4_term = ITERM;
	}

	if (of_property_read_u32(np, "jeita_temp_t2_to_t3_term", &val) >= 0)
		info->data.jeita_temp_t2_to_t3_term = val;
	else {
		chr_err("use default term:%d\n", ITERM);
		info->data.jeita_temp_t1_to_t2_term = ITERM;
	}

	if (of_property_read_u32(np, "jeita_temp_t1_to_t2_term", &val) >= 0)
	...
	if (of_property_read_u32(np, "jeita_temp_t0_to_t1_term", &val) >= 0)
		info->data.jeita_temp_t0_to_t1_term = val;
	else {
		chr_err("use default term:%d\n", ITERM);
		info->data.jeita_temp_t0_to_t1_term = ITERM;
	}

	if (of_property_read_u32(np, "jeita_temp_below_t0_term", &val) >= 0)
		info->data.jeita_temp_below_t0_term= val;
	else {
		chr_err("use default term:%d\n", ITERM);
		info->data.jeita_temp_below_t0_term = ITERM;
	}

	if (of_property_read_u32(np, "jeita_temp_above_t4_recharge", &val) >= 0)
		info->data.jeita_temp_above_t4_recharge = val;
	else {
		chr_err("use default recharge:%d\n", RECHARGE_VOL);
		info->data.jeita_temp_above_t4_recharge = RECHARGE_VOL;
	}

	if (of_property_read_u32(np, "jeita_temp_t3_to_t4_recharge", &val) >= 0)
		info->data.jeita_temp_t3_to_t4_recharge = val;
	else {
		chr_err("use default recharge:%d\n", RECHARGE_VOL);
		info->data.jeita_temp_t3_to_t4_recharge = RECHARGE_VOL;
	}

	if (of_property_read_u32(np, "jeita_temp_t2_to_t3_recharge", &val) >= 0)
		info->data.jeita_temp_t2_to_t3_recharge = val;
	else {
		chr_err("use default recharge:%d\n", RECHARGE_VOL);
		info->data.jeita_temp_t1_to_t2_recharge = RECHARGE_VOL;
	}

	if (of_property_read_u32(np, "jeita_temp_t1_to_t2_recharge", &val) >= 0)
		info->data.jeita_temp_t1_to_t2_recharge= val;
	else {
		chr_err("use default recharge:%d\n", RECHARGE_VOL);
		info->data.jeita_temp_t1_to_t2_recharge = RECHARGE_VOL;
	}

	if (of_property_read_u32(np, "jeita_temp_t0_to_t1_recharge", &val) >= 0)
		info->data.jeita_temp_t0_to_t1_recharge = val;
	else {
		chr_err("use default recharge:%d\n", RECHARGE_VOL);
		info->data.jeita_temp_t0_to_t1_recharge = RECHARGE_VOL;
	}

	if (of_property_read_u32(np, "jeita_temp_below_t0_recharge", &val) >= 0)
		info->data.jeita_temp_below_t0_recharge = val;
	else {
		chr_err("use default recharge:%d\n", RECHARGE_VOL);
		info->data.jeita_temp_below_t0_recharge = RECHARGE_VOL;
	}
	...
	/* charging current */
	if (of_property_read_u32(np, "unknown_charger_current", &val) >= 0) {
		info->data.unknown_charger_current = val;
	} else {
		chr_err("use default UNKNOWN_CHARGER_CURRENT:%d\n",
			UNKNOWN_CHARGER_CURRENT);
		info->data.unknown_charger_current = UNKNOWN_CHARGER_CURRENT;
	}
	if (of_property_read_u32(np, "apple_0_5a_charger_current", &val) >= 0)
		info->data.apple_0_5a_charger_current = val;
	else {
		chr_err("use default APPLE_0_5A_CHARGER_CURRENT:%d\n",
			APPLE_0_5A_CHARGER_CURRENT);
		info->data.apple_0_5a_charger_current =
					APPLE_0_5A_CHARGER_CURRENT;
	}
	...
	if (of_property_read_u32(np, "apple_2_4a_charger_current", &val) >= 0)
		info->data.apple_2_4a_charger_current = val;
	else {
		chr_err("use default APPLE_2_4A_CHARGER_CURRENT:%d\n",
			APPLE_2_4A_CHARGER_CURRENT);
		info->data.apple_2_4a_charger_current =
					APPLE_2_4A_CHARGER_CURRENT;
	}
	...
	if (of_property_read_u32(np, "hvdcp_charger_current", &val) >= 0) {
		...
		chr_err("use default AC_CHARGER_CURRENT:%d\n",
			AC_CHARGER_CURRENT);
		info->data.hvdcp_charger_current = AC_CHARGER_CURRENT;
	}
	...
	if (of_property_read_u32(np, "hvdcp_charger_input_current", &val) >= 0)
		...
		chr_err("use default AC_CHARGER_INPUT_CURRENT:%d\n",
			AC_CHARGER_INPUT_CURRENT);
		info->data.hvdcp_charger_input_current = AC_CHARGER_INPUT_CURRENT;
	}
	...
	if (of_property_read_u32(np, "pd_charger_current", &val) >= 0) {
		...
		chr_err("use default AC_CHARGER_CURRENT:%d\n",
			AC_CHARGER_CURRENT);
		info->data.pd_charger_current = AC_CHARGER_CURRENT;
	}
	...
	if (of_property_read_u32(np, "pd_charger_input_current", &val) >= 0)
		...
		chr_err("use default AC_CHARGER_INPUT_CURRENT:%d\n",
			AC_CHARGER_INPUT_CURRENT);
		info->data.pd_charger_input_current = AC_CHARGER_INPUT_CURRENT;
	}

	if (of_property_read_u32(np, "lowtemp_charger_current", &val) >= 0) {
		info->data.lowtemp_charger_current = val;
	} else {
		chr_err("use default AC_CHARGER_CURRENT:%d\n",
			AC_CHARGER_CURRENT);
		info->data.lowtemp_charger_current = AC_CHARGER_CURRENT;
	}

	if (of_property_read_u32(np, "lowtemp_charger_input_current", &val) >= 0)
		info->data.lowtemp_charger_input_current = val;
	else {
		chr_err("use default AC_CHARGER_INPUT_CURRENT:%d\n",
			AC_CHARGER_INPUT_CURRENT);
		info->data.lowtemp_charger_input_current = AC_CHARGER_INPUT_CURRENT;
	}

	if (of_property_read_u32(np, "hightemp_charger_current", &val) >= 0) {
		info->data.hightemp_charger_current = val;
	} else {
		chr_err("use default AC_CHARGER_CURRENT:%d\n",
			AC_CHARGER_CURRENT);
		info->data.hightemp_charger_current = AC_CHARGER_CURRENT;
	}

	if (of_property_read_u32(np, "hightemp_charger_input_current", &val) >= 0)
		info->data.hightemp_charger_input_current = val;
	else {
		chr_err("use default AC_CHARGER_INPUT_CURRENT:%d\n",
			AC_CHARGER_INPUT_CURRENT);
		info->data.hightemp_charger_input_current = AC_CHARGER_INPUT_CURRENT;
	}
...
void do_sw_jeita_state_machine(struct mtk_charger *info)
{
	...
	if (sw_jeita->sm != TEMP_T2_TO_T3) {
		if (sw_jeita->sm == TEMP_ABOVE_T4) {
			...
			sw_jeita->iterm = info->data.jeita_temp_above_t4_term;
			sw_jeita->recharge_vol = info->data.jeita_temp_above_t4_recharge;
		} else if (sw_jeita->sm == TEMP_T3_TO_T4) {
			sw_jeita->cv = info->data.jeita_temp_t3_to_t4_cv;
			sw_jeita->iterm = info->data.jeita_temp_t3_to_t4_term;
			sw_jeita->recharge_vol = info->data.jeita_temp_t3_to_t4_recharge;
		} else if (sw_jeita->sm == TEMP_T2_TO_T3) {
			sw_jeita->cv = 0;
			sw_jeita->iterm = 0;
			sw_jeita->recharge_vol = 0;
		} else if (sw_jeita->sm == TEMP_T1_TO_T2) {
			sw_jeita->cv = info->data.jeita_temp_t1_to_t2_cv;
			sw_jeita->iterm = info->data.jeita_temp_t1_to_t2_term;
			sw_jeita->recharge_vol = info->data.jeita_temp_t1_to_t2_recharge;
		} else if (sw_jeita->sm == TEMP_T0_TO_T1) {
			sw_jeita->cv = info->data.jeita_temp_t0_to_t1_cv;
			sw_jeita->iterm = info->data.jeita_temp_t0_to_t1_term;
			sw_jeita->recharge_vol = info->data.jeita_temp_t0_to_t1_recharge;
		} else if (sw_jeita->sm == TEMP_BELOW_T0) {
			sw_jeita->cv = info->data.jeita_temp_below_t0_cv;
			sw_jeita->iterm = info->data.jeita_temp_below_t0_term;
			sw_jeita->recharge_vol = info->data.jeita_temp_below_t0_recharge;
		} else {
			sw_jeita->cv = info->data.battery_cv;
			sw_jeita->iterm = info->data.iterm;
			sw_jeita->iterm = info->data.recharge_vol;
		}
	} else {
		sw_jeita->cv = 0;
		sw_jeita->iterm = 0;
	...
	chr_err("[SW_JEITA]preState:%d newState:%d tmp:%d cv:%d,term:%d rechg:%d\n",
		sw_jeita->pre_sm, sw_jeita->sm, info->battery_temp,
		sw_jeita->cv, sw_jeita->iterm, sw_jeita->recharge_vol);
}
...
static int mtk_chgstat_notify(struct mtk_charger *info)
{
	...
	chr_err("%s: notify_code 0x%x, pre_notify_code 0x%x\n", __func__, info->notify_code, info->pre_notify_code);
	if(info->notify_code != info->pre_notify_code)
		ret = kobject_uevent_env(&info->pdev->dev.kobj, KOBJ_CHANGE, env);
	if (ret)
		chr_err("%s: kobject_uevent_fail, ret=%d", __func__, ret);
	else
		info->pre_notify_code = info->notify_code;
...
static ssize_t usb_type_show(struct device *dev, struct device_attribute *attr,
					       char *buf)
{
	struct mtk_charger *pinfo = dev->driver_data;
	union charger_propval usb_type;

	charger_dev_get_property(pinfo->chg1_dev,
			CHARGER_PROP_USB_TYPE, &usb_type);

	chr_err("%s: %d\n", __func__, usb_type.intval);
	return sprintf(buf, "%d\n", usb_type.intval);
}
					       ...
					       char *buf)
{
	...
	chr_err("%s: %d\n", __func__, pinfo->battery_temp / 10);
	return sprintf(buf, "%d\n", pinfo->battery_temp / 10);
}
					       ...
					       char *buf)
{
	...
	union  power_supply_propval battery_present;
	int ret;

	psy = power_supply_get_by_name("mm8013c10-0");
	if (!IS_ERR_OR_NULL(psy)) {
			ret = power_supply_get_property(psy,
				POWER_SUPPLY_PROP_PRESENT, &battery_present);
			if (ret < 0)
				chr_err("get battery present fail\n");
	} else {
			battery_present.intval = 1;
	}

	chr_err("%s: %d\n", __func__, battery_present.intval);
	return sprintf(buf, "%d\n", battery_present.intval);
}

static DEVICE_ATTR_RO(battery_present);

static ssize_t battery_status_show(struct device *dev, struct device_attribute *attr,
					       char *buf)
{
	struct mtk_charger *pinfo = dev->driver_data;
	union  charger_propval battery_status;

	charger_dev_get_property(pinfo->chg1_dev,
			CHARGER_PROP_STATUS, &battery_status);

	chr_err("%s: %d\n", __func__, battery_status.intval);
	return sprintf(buf, "%d\n", battery_status.intval);
}
...
extern struct mm8xxx_bypass_setting *mm8xxx_get_bypass_data(void);
...
int smart_charging(struct mtk_charger *info)
{
	...
	int is_protection = 0, is_protection_eu = 0, protection_high = 0, protection_eu_low = 0, protection_eu_high = 0, protection_low = 0;
	int protection_setting = 0, is_input_suspend = 0, protection_setting_eu = 0, charging_enable = 0;


	chr_err("enter smart_charging\n");
	protection_setting = get_battery_protection_setting();
	protection_setting_eu = get_battery_protection_setting_eu();
	is_input_suspend = get_input_suspend();
	is_protection = protection_setting / 100000;
	is_protection_eu = protection_setting_eu / 100000;
	charging_enable = get_charging_enable();
/* ===== 新增：Bypass 旁路充电核心逻辑 ===== */
    struct mm8xxx_bypass_setting *bps = mm8xxx_get_bypass_data();
    static bool is_bypass_charging_phase = false; // 用于记录是否处于"正在向high充电"的状态

    if (bps && bps->enable) {
        int current_soc = sc_battery_percentage / 100;

        // 状态机区间判定
        if (current_soc < bps->low_capacity) {
            is_bypass_charging_phase = true;
        } else if (current_soc >= bps->high_capacity) {
            is_bypass_charging_phase = false;
        }

        // 依据当前阶段覆盖控制变量
        // 注：在 Lenovo sysfs 逻辑中，charging_enable=1 实际上意味着"禁用充电"
        if (is_bypass_charging_phase) {
            // 低于 low_capacity，持续充电直到 high_capacity
            charging_enable = 0; // 允许充电
            is_input_suspend = 0; // 允许输入
        } else {
            if (current_soc >= bps->high_capacity) {
                // 高于 high_capacity，仅电池供电（断开外部电源和充电）
                charging_enable = 1; // 停止充电
                is_input_suspend = 1; // 挂起输入，开启纯电池模式
            } else {
                // 在 low 和 high 之间，旁路模式（仅电源适配器供电）
                charging_enable = 1; // 停止充电
                is_input_suspend = 0; // 保持输入不挂起
            }
        }
        
        chr_err("[Bypass] soc:%d, phase:%d, chg_disable:%d, susp:%d\n",
                current_soc, is_bypass_charging_phase, charging_enable, is_input_suspend);
    }
    /* ======================================= */

	if(is_protection) {
		protection_low = (protection_setting - 100000) / 1000;
		protection_high = (protection_setting - 100000 - protection_low * 1000);
		if(protection_low < 40)
			protection_low = 40;
		if(protection_high < 60)
			protection_high = 60;
	}
	if(is_protection_eu) {
		protection_eu_low = (protection_setting_eu - 100000) / 1000;
		protection_eu_high = (protection_setting_eu - 100000 - protection_eu_low * 1000);
		protection_eu_low = 0;
		if (protection_eu_high < 80)
			protection_eu_high = 80;
	}
	if(is_input_suspend && !info->sc.chg_dev_chgen) {
		info->sc.disable_charger = true;
		info->sc.chg_dev_chgen = true;
		info->sc.chg_dev_chgen_eu = false;
		charger_dev_enable(info->chg1_dev, false);
		...
		chr_err("input suspend discharge\n");
	} else if (is_protection && !is_input_suspend) {
		if(sc_battery_percentage >= protection_high * 100 && !info->sc.chg_dev_chgen  && !info->sc.disable_charger) {
			info->sc.disable_charger = true;
			info->sc.chg_dev_chgen = true;
			info->protection_dischg = true;
			charger_dev_enable(info->chg1_dev, false);
			if (info->algo.do_algorithm)
				info->algo.do_algorithm(info);
			mtk_charger_force_disable_power_path(info, CHG1_SETTING, true);
			charger_dev_do_event(info->chg1_dev,
					EVENT_DISCHARGE, 0);
			ret_value = SC_DISABLE;
			chr_err("protection discharge\n");
		} else if(sc_battery_percentage <= protection_low * 100 && info->sc.disable_charger) {
			info->sc.disable_charger = false;
			info->sc.chg_dev_chgen = false;
			mtk_charger_force_disable_power_path(info, CHG1_SETTING, false);
			charger_dev_enable(info->chg1_dev, true);
			charger_dev_do_event(info->chg1_dev,
					EVENT_RECHARGE, 0);
			ret_value = SC_REDUCE;
			chr_err("protection discharge recovery\n");
		}
		if(sc_battery_percentage > protection_low * 100
			&& sc_battery_percentage <= protection_high * 100
			&& info->sc.chg_dev_chgen) {
			info->sc.chg_dev_chgen = false;
			if (info->algo.do_algorithm)
				info->algo.do_algorithm(info);
			ret_value = SC_DISABLE;
			chr_err("protection restore online current\n");
		}
		info->sc.chg_dev_chgen_eu = false;
	} else if((is_protection_eu || charging_enable) && !is_input_suspend && !is_protection) {
		if(((sc_battery_percentage >= protection_eu_high * 100 &&  protection_eu_high != 100) || charging_enable) && !info->sc.chg_dev_chgen_eu) {
			info->sc.disable_charger = true;
			info->sc.chg_dev_chgen = false;
			info->protection_dischg = false;
			if (info->algo.do_algorithm)
				info->algo.do_algorithm(info);
			info->sc.chg_dev_chgen_eu = true;
			charger_dev_enable(info->chg1_dev, false);
			charger_dev_do_event(info->chg1_dev,
					EVENT_DISCHARGE, 0);
			ret_value = SC_DISABLE;
			chr_err("protection_eu suspend charge\n");
		} else {
			if ((info->sc.chg_dev_chgen || info->sc.disable_charger)
				&& ((sc_battery_percentage <= protection_eu_high * 95 && !charging_enable) || info->protection_dischg)) {
				info->sc.disable_charger = false;
				info->sc.chg_dev_chgen = false;
				charger_dev_enable(info->chg1_dev, true);
				if (info->algo.do_algorithm)
				...
				ret_value = SC_REDUCE;
				info->sc.chg_dev_chgen_eu = false;
				chr_err("protection_eu restore charge\n");
			} else {
				info->sc.chg_dev_chgen = false;
				chr_err("IGNORE\n");
			}
		}
	} else if(!is_protection_eu && !is_input_suspend && !is_protection && (info->sc.chg_dev_chgen || info->sc.disable_charger) && !charging_enable) {
		info->sc.disable_charger = false;
		info->sc.chg_dev_chgen = false;
		if (info->algo.do_algorithm)
			info->algo.do_algorithm(info);
		...
		charger_dev_do_event(info->chg1_dev,
				EVENT_RECHARGE, 0);
		ret_value = SC_REDUCE;
		chr_err("input suspend discharge recovery\n");
	} else {
		chr_err("IGNORE discharge\n");
		ret_value = SC_IGNORE;
	}
	chr_err("[sc]disable_charger[%d], sc_battery_percentage[%d], protection_high[%d], protection_low[%d], \
		protection_setting[%d], protection_eu_high[%d], protection_eu_low[%d], \
		protection_setting_eu[%d], chg_dev_chgen[%d], is_input_suspend[%d], charging_enable[%d], ret_value[%d]\n",
		info->sc.disable_charger, sc_battery_percentage, protection_high, protection_low,
		protection_setting, protection_eu_high, protection_eu_low,
		protection_setting_eu, info->sc.chg_dev_chgen, is_input_suspend, charging_enable, ret_value);
	return ret_value;
...
bool is_kernel_power_off_charging_tp(void)
{
	static struct mtk_charger *pinfo;
	struct power_supply *psy;
...
EXPORT_SYMBOL(is_kernel_power_off_charging_tp);
...
static bool mtk_chg_check_vbus(struct mtk_charger *info)
{
	...
	if (vchr > info->data.pd_max_charger_voltage) {
...
static int mtk_charger_plug_out(struct mtk_charger *info)
{
	...
	info->curr_flag = false;
	info->old_vreg = false;
	info->pre_notify_code = 0;
				...
				int chr_type)
{
	...
	if(info->chr_type == POWER_SUPPLY_TYPE_USB_DCP &&
			info->usb_type == POWER_SUPPLY_USB_TYPE_PD) {
...
static bool mtk_is_charger_on(struct mtk_charger *info)
{
	...
	chr_err("mtk_is_charger_on enter\n");
...
static void kpoc_power_off_check(struct mtk_charger *info)
{
	...
	if (boot_mode == 8 || boot_mode == 9) {
		...
		if (vbus >= 0 && vbus < 2500 && !mtk_is_charger_on(info) &&
			(!info->ta_hardreset ||  !info->pd_reset)) {
...
{
	union charger_propval online = {0}, status = {0};
	int ret = 0;
	bool charging = true;
	...
	if(info == NULL)
		return;
#if(0)
	chg_psy = power_supply_get_by_name("sc-charger");
...
#endif
	ret = charger_dev_get_property(info->chg1_dev,
			CHARGER_PROP_ONLINE, &online);
		if (ret < 0)
			chr_debug("%s: %d\n", __func__, ret);

	ret = charger_dev_get_property(info->chg1_dev,
			CHARGER_PROP_STATUS, &status);
		if (ret < 0)
			chr_debug("%s: %d\n", __func__, ret);
	...
	if (charging != info->is_charging && !IS_ERR_OR_NULL(info->psy1))
...
static int charger_routine_thread(void *arg)
{
	...
	while (1) {
		...
		if(ui_soc != pre_ui_soc) {
			pre_ui_soc = ui_soc;
			chr_err("update charging level, pre_ui_soc = %d\n", pre_ui_soc);
			power_supply_changed(info->psy1);
		}
...
static int sysfs_notify_event(struct notifier_block *notifier,
			unsigned long event, void *unused)
{
	...
	info = container_of(notifier,
		struct mtk_charger, sysfs_notifier);
	
	if(!info)
		return NOTIFY_DONE;
	charger_dev_get_property(info->chg1_dev,
			CHARGER_PROP_ONLINE, &online);
	if (!online.intval)
		return NOTIFY_DONE;
	switch(event) {
...
static int mtk_charger_setup_files(struct platform_device *pdev)
{
	...
	ret = device_create_file(&(pdev->dev), &dev_attr_usb_type);
	if (ret)
		goto _out;
	...
	ret = device_create_file(&(pdev->dev), &dev_attr_battery_status);
	if (ret)
		goto _out;
...
static enum power_supply_property ac_psy_properties[] = {
	POWER_SUPPLY_PROP_ONLINE,
};
...
{
	struct mtk_charger *info;
	union charger_propval prop = {0};
	union charger_propval prop2 = {0};
	int ret = 0;
	static int pre_usb_type = 0;
...
#endif
	ret = charger_dev_get_property(info->chg1_dev,
			CHARGER_PROP_ONLINE, &prop);
	if (ret < 0)
		chr_debug("%s: %d\n", __func__, ret);

	ret = charger_dev_get_property(info->chg1_dev,
			CHARGER_PROP_USB_TYPE, &prop2);
	if (ret < 0)
		chr_debug("%s: %d\n", __func__, ret);
	...
	pr_notice("%s event, name:%s online:%d type:%d vbus:%d\n", __func__,
		...
		get_vbus(info));
	charger_status_check(info);
	if(pre_usb_type != prop2.intval && prop2.intval != 0) {
		if(!IS_ERR_OR_NULL(info->psy1))
			power_supply_changed(info->psy1);
		info->sc.chg_dev_chgen  = false;
		info->sc.disable_charger = false;
		info->sc.chg_dev_chgen_eu = false;
	...
	pre_usb_type = prop2.intval;
	_wake_up_charger(info);
}

int get_pd_adapter(int pd_type) {
	int pd_adapter = 0;
	switch (pd_type) {
	case TA_ATTACH:
		pd_adapter = POWER_SUPPLY_USB_TYPE_PD;
		break;
	default:
		pd_adapter = 0;
		break;
			...
			unsigned long evt, void *val)
{
	...
	case MTK_SINK_VBUS:
		...
		break;
	case MTK_PD_ADAPTER_RESET_STATUS:
		chr_err(" pd adapter reset \n");
		pinfo->ta_status[index] = TA_ATTACH;
		break;
	}
	chr_debug("%s: evt: pd:%d, ufcs:%d\n", __func__,
	pinfo->ta_status[PD], pinfo->ta_status[UFCS]);
	pd_adapter = get_pd_adapter(pinfo->ta_status[PD]);
	chr_err("%s: pd_adapter:%d\n", __func__, pd_adapter);
	if (pinfo != NULL && (pinfo->bootmode == 8 || pinfo->bootmode == 9) && pd_adapter != 0) {
		do {
			if (pinfo->chg1_dev == NULL || pinfo->chg1_dev->ops == NULL) {
				mdelay(100);
			} else {
				charger_dev_set_pd_adapter(pinfo->chg1_dev, &pd_adapter);
				break;
                        }
		} while(1);
	} else {
...
static int mtk_charger_probe(struct platform_device *pdev)
{
	...
	info->data.chan_vbus = devm_iio_channel_get(&pdev->dev, "pmic_vbus");
	if (IS_ERR_OR_NULL(info->data.chan_vbus))
		chr_err("%s: devm power fail to chan_vbus\n", __func__);
	...
	info->chg_psy = power_supply_get_by_name("sc-charger");
	if (IS_ERR_OR_NULL(info->chg_psy)) {
		info->chg_psy = power_supply_get_by_name("sgm4154x-charger");
		if(IS_ERR_OR_NULL(info->chg_psy))
	...
	info->bat_psy = power_supply_get_by_name("mm8013c10-0");
	...
	info->psy_dvchg_desc1.name = "ac";
	info->psy_dvchg_desc1.type = POWER_SUPPLY_TYPE_MAINS;
	info->psy_dvchg_desc1.properties = ac_psy_properties;
	info->psy_dvchg_desc1.num_properties =
		ARRAY_SIZE(ac_psy_properties);
	info->psy_dvchg_desc1.get_property = mt_ac_get_property;
	...
	info->psy_dvchg_desc2.name = "usb";
	info->psy_dvchg_desc2.type = POWER_SUPPLY_TYPE_USB;
	info->psy_dvchg_desc2.properties = usb_psy_properties;
	info->psy_dvchg_desc2.num_properties =
		ARRAY_SIZE(usb_psy_properties);
	info->psy_dvchg_desc2.get_property = mt_usb_get_property;
	...
	if (IS_ERR(info->psy_dvchg2))
		...
		lenovo_charge_create_sysfs(info->psy_dvchg2);
		sysfs_register_notifier(&info->sysfs_notifier);
		info->sysfs_notifier.notifier_call = sysfs_notify_event;
	}
