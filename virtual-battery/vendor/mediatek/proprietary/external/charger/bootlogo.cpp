...
void bootlogo_show_charging(int capacity, int cnt)
{
    ...
    battery_present = get_battery_online_status();
    is_float = get_float_charging_state();
    
    if (battery_present != true)
	{
		show_charging_no_battery();
		return;
	}
    ...
    if (get_battnotify_status())
    {
		...
		{
			show_battery_temperature_low();
			return;
		}
		else if (battery_temperature >= 50)
		{
			show_battery_temperature_high();
			return;
		}
		return;
    }
	...
	/*add by andy*/
	if (get_bypass_setting()) {
        KPOC_LOGI("oh!yesyesyesyesyesyesyes!\n");
        ...
        KPOC_LOGI("oh!nonononononononononononononononono\n");
    }
	...
	if (is_charging_source_available() && is_charging_source_status() == 2)
	{
		KPOC_LOGI("[ChargingAnimation: %s %d]show_battery_capacity %d\n",__FUNCTION__,__LINE__, capacity);
		show_battery_capacity(capacity);
	} else {
		if(battery_present != true) {
			show_charging_no_battery();
		} else {
			show_plug_out_capacity(capacity);
			KPOC_LOGI("[ChargingAnimation: %s %d]show_plug_out_capacity %d\n",__FUNCTION__,__LINE__, capacity);
		}
	}
}
