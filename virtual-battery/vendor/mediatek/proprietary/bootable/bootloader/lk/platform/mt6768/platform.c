...
static bool readBypassEnable(bool *bypass_en)
{
    ...
    dprintf(CRITICAL, "[readBypassEnable] entry\n");

    if (!bypass_en) {
        dprintf(CRITICAL, "[readBypassEnable] bypass_en is NULL\n");
        return false;
    }

    // 首先尝试通过 bio 层读取
    dprintf(CRITICAL, "[readBypassEnable] calling bio_open(%s)\n", PRO_INFO_NAME);
    bdev = bio_open(PRO_INFO_NAME);
    if (bdev != NULL) {
        dprintf(CRITICAL, "[readBypassEnable] bio_open success, using bio_read\n");
        len = bio_read(bdev, (u8 *)&code, NV_BYPASSENABLE_OFFSET, NV_BYPASSENABLE_LEN);
        bio_close(bdev);
        if (len >= NV_BYPASSENABLE_LEN) {
            *bypass_en = (code != 0);
            dprintf(CRITICAL, "[readBypassEnable] bio_read success, code=0x%x, bypass_en=%d\n", code, *bypass_en);
            return true;
        } else {
            dprintf(CRITICAL, "[readBypassEnable] bio_read failed, len=%d\n", len);
            return false;
        }
    } else {
        dprintf(CRITICAL, "[readBypassEnable] bio_open failed, falling back to direct mmc read\n");
    }

    // 后备：直接使用 mmc_block_read 读取
    // 从 GPT 日志获取 proinfo 起始扇区 0x1bd000
    unsigned long proinfo_start_sector = 0x1bd000;
    // 计算目标字节所在的扇区号和扇区内偏移
    unsigned int sector = proinfo_start_sector + (NV_BYPASSENABLE_OFFSET / 512);
    unsigned int offset_in_sector = NV_BYPASSENABLE_OFFSET % 512;
    // 缓冲区需对齐，使用 __attribute__((aligned(4)))
    u8 sector_buf[512] __attribute__((aligned(4)));

    int ret = mmc_block_read(0, sector, 1, (unsigned long *)sector_buf);
    if (ret == 0) {
        code = sector_buf[offset_in_sector];
        *bypass_en = (code != 0);
        dprintf(CRITICAL, "[readBypassEnable] mmc_block_read success, code=0x%x, bypass_en=%d\n", code, *bypass_en);
        return true;
    } else {
        dprintf(CRITICAL, "[readBypassEnable] mmc_block_read failed, ret=%d\n", ret);
        return false;
    }
}
//add by andy
		...
		for (tags = (void *)BOOT_ARGUMENT_LOCATION; tags->hdr.size; tags = boot_tag_next(tags)) {
			switch (tags->hdr.tag) {
				...
				case BOOT_TAG_SOC_ID:
					...
					break;
				case BOOT_TAG_ROM_INFO:
					m_sec_usb_dl = tags->u.rom_info.m_sec_usb_dl;
					m_sec_boot = tags->u.rom_info.m_sec_boot;
					m_seccfg_ac_en = tags->u.rom_info.m_seccfg_ac_en;
					break;
				default:
					break;
			}
...
void sw_env()
{
	...
	switch (g_boot_mode) {
		case META_BOOT:
			...
			if (g_boot_arg -> meta_com_type != 1)
				g_boot_arg -> meta_com_type =2;
			mt_disp_show_boot_meta_logo();
			break;
		case FACTORY_BOOT:
		...
		case FASTBOOT:
			...
			mt_disp_show_boot_fastboot_logo();
			break;
		case BOOT_MENU:
			video_printf(" => BOOT_MENU mode...\n");
			mt_disp_show_boot_menu_logo();
			break;
		default:
	...
	if (g_boot_mode == BOOT_MENU) {
		//video_printf(" => USER BOOT_MENU mode...\n");
		dprintf(CRITICAL, " USER BOOT_MENU MODE\n");
		mtk_wdt_disable();
		mt_disp_show_boot_menu_logo();
		boot_menu_select();
		mtk_wdt_init();
	...
	if (g_boot_mode == META_BOOT) {
		//video_printf(" => USER META MODE\n");
		dprintf(CRITICAL, " USER META MODE\n");
		if (g_boot_arg -> meta_com_type != 1)
			g_boot_arg -> meta_com_type =2;
		mt_disp_show_boot_meta_logo();
	}
...
#ifndef MTK_POL_DEPRECATED
#ifdef MTK_SYSIRQ_INIT_LK
	mt_irq_sysirq_init();
#endif
#endif
...
void platform_init(void)
{
	...
	/*
	dprintf(CRITICAL, "=== Starting virtual battery test ===\n");
	
	mt_disp_show_vitural_battery();
	mt65xx_backlight_on();  // 确保背光打开
	
	mdelay(8000);           // 停留 8 秒观察
	dprintf(CRITICAL, "=== Virtual battery test finished ===\n");                   // 等待 5 秒，便于观察
	*/
...
// 添加启动模式判断，仅在正常启动或闹钟启动时执行 bypass 逻辑
if (g_boot_mode == NORMAL_BOOT || g_boot_mode == ALARM_BOOT) {
    dprintf(CRITICAL, "=== Starting virtual battery decide ===\n");
    res = getBypassSetting(&bypass_en);
    if (!res) {
        dprintf(CRITICAL, "platform_init: get bypass_en failed, assume bypass disabled\n");
        bypass_en = false;  // 设置默认值
        mt_disp_show_boot_logo();
        // 不跳转，继续执行后续初始化
    }
    // 获取并打印当前电压
unsigned int volt = get_chr_volt();
dprintf(CRITICAL, "Current battery voltage = %u mV\n", volt);

if (bypass_en && volt < 3000) {
        dprintf(CRITICAL, "platform_init: bypass_en = %d, exec bypass (volt unignored)\n", bypass_en);
        mt_disp_show_vitural_battery();
        mdelay(4000);
        chr_power_off();
    } else {
        dprintf(CRITICAL, "platform_init: bypass_en = %d, normal boot\n", bypass_en);
        mt_disp_show_boot_logo();
    }
} else {
    dprintf(CRITICAL, "platform_init: boot_mode=%d, skip bypass logic\n", g_boot_mode);
    mt_disp_show_boot_logo();
}
dprintf(CRITICAL, "=== Virtual battery decide finished ===\n");
