...
#define BOOT_MENU_INDEX   99
#define BOOT_FASTBOOT_INDEX   100
#define BOOT_META_INDEX   101
...
/*For meta*/
void mt_disp_show_boot_meta_logo(void)
{
	dprintf(INFO, "[meta logo: %s %d]\n",__FUNCTION__,__LINE__);
	mt_logo_get_custom_if();

	init_fb_screen();
	fill_animation_logo(BOOT_META_INDEX, mt_get_fb_buf(), (void *)mt_get_tempfb_addr(), logo_addr, phical_screen);
	mt_disp_update(0, 0, CFG_DISPLAY_WIDTH, CFG_DISPLAY_HEIGHT);
...
/*For fastboot*/
void mt_disp_show_boot_fastboot_logo()
{
	dprintf(INFO, "[fastboot logo: %s %d]\n",__FUNCTION__,__LINE__);
	mt_logo_get_custom_if();

	init_fb_screen();
	fill_animation_logo(BOOT_FASTBOOT_INDEX, mt_get_fb_buf(), (void *)mt_get_tempfb_addr(), logo_addr, phical_screen);
	mt_disp_update(0, 0, CFG_DISPLAY_WIDTH, CFG_DISPLAY_HEIGHT);
...
/*For boot menu*/
void mt_disp_show_boot_menu_logo()
{
	dprintf(INFO, "[boot menu logo: %s %d]\n",__FUNCTION__,__LINE__);
	mt_logo_get_custom_if();

	init_fb_screen();
	fill_animation_logo(BOOT_MENU_INDEX, mt_get_fb_buf(), (void *)mt_get_tempfb_addr(), logo_addr, phical_screen);
	mt_disp_update(0, 0, CFG_DISPLAY_WIDTH, CFG_DISPLAY_HEIGHT);
...
void mt_disp_show_low_battery(void)
{
 ...
 //add by andy
void mt_disp_show_vitural_battery(void)
{
    init_fb_screen();                     // 初始化 phical_screen 等参数
    display_hardcoded_logo(mt_get_fb_buf());  // 传入帧缓冲地址
}
