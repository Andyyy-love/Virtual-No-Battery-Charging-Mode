...
static char mmc_wp_cmdline_buf1[CMDLINE_BUF_SIZE] = { 0 };
static char mmc_wp_cmdline_buf2[CMDLINE_BUF_SIZE] = { 0 };
...
unsigned int mmc_set_write_protect(int dev_num, Region partition, unsigned long blknr, u32 blkcnt, STORAGE_WP_TYPE type)
{
	...
	if(partition == EMMC_PART_USER) {
		...
		mmc_commandline_set_wp(partition,blknr, blkcnt);
...
/*
 * Setting kernel command line of write protect group info
 * override by platform
 */

static void mmc_commandline_set_wp_for_boot(u32 id)
{
    if (id == 1)
        strcat(mmc_wp_cmdline_buf1,"mtk-mmc-wp.wp_boot1=1");
    else if (id == 2)
        strcat(mmc_wp_cmdline_buf1,"mtk-mmc-wp.wp_boot2=1");
}

static void mmc_commandline_set_wp_for_user(unsigned long blknr, u32 blkcnt)
{
    int ret = 0;
	char temp[CMDLINE_BUF_SIZE] = { 0 };

    if (!strlen(mmc_wp_cmdline_buf2)) {
        ret = snprintf(mmc_wp_cmdline_buf2, CMDLINE_BUF_SIZE, "mtk-mmc-wp.wp_user=0x%lx,0x%x", blknr, blkcnt);
        if (ret < 0) {
            dprintf(ALWAYS, "mmc append wp info to command line fail\n");
			return;
        }
    } else {
		memset(temp, 0, CMDLINE_BUF_SIZE);
        ret = snprintf(temp, CMDLINE_BUF_SIZE, ",0x%lx,0x%x", blknr, blkcnt);
        if (ret < 0) {
            dprintf(ALWAYS, "mmc append wp info to command line fail\n");
            return;
        }
		if ((strlen(temp) + strlen(mmc_wp_cmdline_buf2)) < CMDLINE_BUF_SIZE)
			strcat(mmc_wp_cmdline_buf2, temp);
		else
			dprintf(ALWAYS, "not enough mmc_wp_cmdline_buf2:%d,%s,%d,%s\n", strlen(mmc_wp_cmdline_buf2), mmc_wp_cmdline_buf2, strlen(temp), temp);
    }
}

void mmc_commandline_set_wp(u32 id, unsigned long blknr, u32 blkcnt)
{
    switch (id) {
    case 0:
        mmc_commandline_set_wp_for_user(blknr, blkcnt);
        break;
    case 1:
    case 2:
        mmc_commandline_set_wp_for_boot(id);
        break;
    default:
        break;
    }
...
void mmc_commandline_set_wp_append(void)
{
	cmdline_append(mmc_wp_cmdline_buf1);
	cmdline_append(mmc_wp_cmdline_buf2);
}
...
{
	return mmc_init(1, MSDC_MODE_DEFAULT);
}

int sdcard_deinit(void)
{
	return msdc_deinit(&sd_host[1]);
}

int sdcard_block_write(unsigned long blknr, u32 blkcnt, unsigned long *src)
{
	unsigned long ret;
	...
	if (mmc_block_write(1, blknr, blkcnt, src) == MMC_ERR_NONE)
		ret = blkcnt;
	else
		ret = (unsigned long) -1;
...
#ifdef LK_PROFILING
	t2 = get_timer(t1);
	msdc_pr_err("[PROFILE] sdcard write %lu blks in %lu ms: %luKB/s\n",
...
int sdcard_block_read(unsigned long blknr, u32 blkcnt, unsigned long *dst)
{
	unsigned long ret;
	...
	if (mmc_block_read(1, blknr, blkcnt, dst) == MMC_ERR_NONE)
		ret = blkcnt;
	else
		ret = (unsigned long) -1;
...
#ifdef LK_PROFILING
	t2 = get_timer(t1);
	msdc_pr_err("[PROFILE] sdcard read %lu blks in %lu ms: %luKB/s\n",
		blkcnt, t2, blkcnt * 500 / t2);
#endif
